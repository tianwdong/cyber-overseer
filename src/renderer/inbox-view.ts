...
export function renderInbox(s:OverseerState){
 ...
 get('inbox-list').replaceChildren(...[...entries].reverse().map(entry=>{
  ...
  card.append(heading,meta,excerpt,button);card.addEventListener('toggle',()=>{if(card.open&&!entry.read)void window.overseer.command('inbox-read',entry.id).catch(()=>{get('inbox-error').hidden=false;get('inbox-error').textContent=en?'Unable to save read status.':'已读状态暂时无法保存。';});});return card;
 }));
}
