...
function populate(){(document.getElementById('auto-update-check') as HTMLInputElement).checked=current.checkForUpdates!==false;(document.getElementById('notify-attention') as HTMLInputElement).checked=current.notifyOnAttention!==false;enabled.checked=current.retryAfterFailure;limit.value=String(current.maxAttempts);limit.disabled=!enabled.checked;language.value=current.language;}
...
document.getElementById('settings-form')!.onsubmit=async event=>{
  ...
  try{const state=await window.overseer.command('settings',JSON.stringify({checkForUpdates:(document.getElementById('auto-update-check') as HTMLInputElement).checked,notifyOnAttention:(document.getElementById('notify-attention') as HTMLInputElement).checked,retryAfterFailure:enabled.checked,maxAttempts:Number(limit.value),language:language.value}));updateSettings(state);populate();document.getElementById('settings-status')!.textContent=tr('设置已保存',uiLanguage);}
